    ...
    <div class="container-fluid">
        ...
        <div class="box-body">
			...
			<?php
				...
				$PHARMACIE="";
				$DATEAJOUT="";
				$req="select * from medicament order by id desc";
				$query=mysqli_query($connect, $req);
				while($enreg=mysqli_fetch_array($query)){
					$NOM	=	$enreg['NOM'];
					$DISPONIBILITE		=	$enreg['DISPONIBLE'];
					$UNITE		=	$enreg['UNITE'];
					$PHARMACIE		=	$enreg['PHARMACIE'];
					$TYPE	=	$enreg['TYPE'];
					$DATEAJOUT	=	$enreg['DATEAJOUT'];

      $nom_pharmacie="";
      $rec="select * from pharmacie where id=".$PHARMACIE;
      $query1=mysqli_query($connect, $rec);
      while ($res=mysqli_fetch_array($query1)) {
      $nom_pharmacie= $res["NOMPHARMACIE"];
     }
       ?>
					...
					<tr>
							...
							<td style="color:#333"><?php echo $DISPONIBILITE; ?></td>
							<td style="color:#333"><?php echo $UNITE; ?></td>
							<td style="color:#333"><?php echo $TYPE; ?></td>
							<td style="color:#333"><?php echo $nom_pharmacie; ?></td>
        ...
        <div class="row clearfix">
            <div class="col-sm-12 text-center">
                <a href="add-medicament.php" class="btn btn-raised g-bg-cyan">Ajoute medicament</a>
            </div>
        </div>
    </div>
</section>
